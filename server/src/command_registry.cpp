...
void voxeng::server::CommandRegistry::registerCommand(const std::string& name, CommandHandler handler) {
    m_commands.insert({ name, handler });
}


bool voxeng::server::CommandRegistry::tryExecute(const std::string& input, CommandContext& ctx) {
    if (input.length() == 0) {
        return false;
    }
